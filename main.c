...
void save()//KAYIT ISLEMI BU FONKSIYON ILE YAPILIYOR
{
...
void olustur()
{//BU FONKSIYON ILE FARE KLAVYE KONTROLLER AKTIF EDILIYOR
    int derinlik;
    allegro_init();
...
void faredenetim()
{
    ...
    else if (mouse_b==1)
    {
        ...
        else if(sec==8)///  çember cizme-boS
        {
            while(1)
            {
                if(mouse_b!=1)
...
void ekran()
{
    ...
    clear_to_color(bmp,makecol(250, 211, 155));/// olusan ekranin arkasindaki renk
    ciz();
    boya();
    rectfill(bmp,70,70,1000,600,x);///ekran üzerindeki her kismi beyaz yapar(ikincibir renk olarak)
}
void boya()///boya kutusu olusturma
{
...
void farearac()
{
    if(mouse_b==1) ///koordinatlara gore mouse tiklandiginda o islemi yapmasini saglama orn: x>841 ve x<900 koord. tikladndgnda cikis yap gbi..
    {
...
void ciz() ///ekrani ust satiri ve sol satiri olarak ayarlama
{
...
///ust menunun araclari kalem silgi dikdortgen ucgen vb..
    ...
    textprintf_centre_ex(bmp, font,770, 35, makecol(0, 0, 0),-1,"MUHAMMED SARA");
    ...
    /// sol menunun araclari kaydet renk boyama vb..
